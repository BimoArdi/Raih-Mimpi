...
//	@ResponseBody
	public String formInsertSponsor (@RequestParam("namap")String namaProyek,@RequestParam(value="username",required=false)String nama,@RequestParam("bank")Bank b,
		...
		if (l != null) {
			Pengguna pengguna = pgs.getByLogin(l.getId());
		...
		proyek.setSisaDana((ss.getTotalSponsor(proyek.getId())-pds.getByProyek(proyek.getId())));
		ps.saveOrUpdate(proyek);
		ns.saveSponsor();
		return "redirect:/detailsponsor?id="+s.getId();
//		return pds.getByProyek(proyek.getId());
	}	
